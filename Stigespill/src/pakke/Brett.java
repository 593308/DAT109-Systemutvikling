...
public class Brett {
	...
	/**
	 * genererer en tilfeldig rute på brettet
	 * 
	 */
	...
	public int sjekkRute(int rute) {
		if (slangerOgStiger.containsKey(rute)) {
			if (slangerOgStiger.get(rute) > rute)
				System.out.println(
						"Grattis! Du har landet p� en stige og blir flyttet til plass " + slangerOgStiger.get(rute));
			else
				System.out.println("Wooops, du har havnet p� en slange og blir flyttet tilbake til plass "
